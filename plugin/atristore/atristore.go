...
func init() {
	...
	*/
	engine.OnRegex(`^#购买([^ ]*)\s*(\d*)$`, getdb).SetBlock(true).Limit(ctxext.LimitByUser).Handle(func(ctx *zero.Ctx) {
		...
		ctx.Send(message.ReplyWithMessage(ctx.Event.MessageID, message.Text("你用", price, "W。购买了", strconv.Itoa(number), "个《"+thingName, "》\n已通知管理员为你发货\n请将此聊天记录截图保存作为凭证")))
	})
}
